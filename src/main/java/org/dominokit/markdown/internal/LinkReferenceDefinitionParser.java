...
public class LinkReferenceDefinitionParser {
  ...
  public void parse(SourceLine line) {
    ...
    if (state == State.PARAGRAPH) {
      // We're in a paragraph now. Link reference definitions can only appear at the beginning, so
      // once
    ...
    while (scanner.hasNext()) {
      ...
      if (!success) {
        ...
        // If parsing of the title part failed, we still have a valid reference that we can add, and
        // we need to
