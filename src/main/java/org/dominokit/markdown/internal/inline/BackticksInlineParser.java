  ...
  public ParsedInline tryParse(InlineParserState inlineParserState) {
    ...
    while (scanner.find('`') > 0) {
      ...
      if (count == openingTicks) {
        ...
        // spec: If the resulting string both begins and ends with a space character, but does not
        // consist
        // entirely of space characters, a single space character is removed from the front and
        // back.
        if (content.length() >= 3
            && content.charAt(0) == ' '
